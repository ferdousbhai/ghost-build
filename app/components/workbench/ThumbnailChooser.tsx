...
export async function uploadThumbnail(imageData: string, sessionId: string, chatId: string): Promise<void> {
  ...
  const uploadResponse = await fetch(`${siteUrl}/upload_thumbnail?sessionId=${sessionId}&chatId=${chatId}`, {
    method: 'POST',
    body: blob,
  });
...
export function ThumbnailChooser({ isOpen, onOpenChange, onRequestCapture }: ThumbnailChooserProps) {
  ...
  const previewImage = localPreview || lastUploadedPreview || (!recentlyUploaded ? currentThumbnail : null);
