...
export const FileTree = memo(
  ({
    ...
    const [collapsedFolders, setCollapsedFolders] = useState(() => {
      const allFolders = folderPathsFromFileList(fileList);
      if (collapsed) {
...
function folderPathsFromFileList(fileList: Node[]) {
  return fileList.filter((item) => item.kind === 'folder').map((item) => item.fullPath);
}
...
function sortFileList(rootFolder: string, nodeList: Node[], hideRoot: boolean): Node[] {
  ...
  const rootChildren = hideRoot ? childrenMap.get(rootFolder) || [] : [];
  if (!hideRoot) {
