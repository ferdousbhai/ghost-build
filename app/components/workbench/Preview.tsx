...
type ResizeHandleSide = Exclude<ResizeSide, null>;
...
export const Preview = memo(function Preview({ showClose, onClose }: { showClose: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (proxyBaseUrl.endsWith('/')) {
      captureMessage('proxyBaseUrl unexpectedly has a trailing slash');
    }
  ...
  useEffect(() => {
    if (previews.length > 1 && !hasSelectedPreview.current) {
      const minPortIndex = previews.reduce(
        (minIndex, preview, index) => (preview.port < previews[minIndex].port ? index : minIndex),
        0,
      );
  ...
  const startResizing = (e: ReactMouseEvent, side: ResizeSide) => {
    if (!isDeviceModeOn) {
      return;
    }
  ...
  const onMouseMove = (e: MouseEvent) => {
    ...
    const direction = resizingState.current.side === 'left' ? -1 : 1;
    const newWidthPercent = Math.max(10, Math.min(resizingState.current.startWidthPercent + direction * dxPercent, 90));
  ...
  const openInNewWindow = async () => {
    ...
    if (!match) {
      logger.warn('Invalid WebContainer URL:', proxyBaseUrl);
      return;
    }
          ...
          <input
            ...
            onKeyDown={(event) => {
              if (proxyBaseUrl === null) {
                captureMessage('key down event received while proxyBaseUrl is null');
                return;
              }
        ...
        >
          {activePreview ? (
            proxyBaseUrl ? (
              ...
              <div className="text-content-primary flex size-full items-center justify-center bg-bolt-elements-background-depth-1">
                <div>
                  <Spinner />
                </div>
              </div>
          ...
          {isDeviceModeOn && (
            <>
              <ResizeHandle side="left" onMouseDown={startResizing} />
              <ResizeHandle side="right" onMouseDown={startResizing} />
            </>
          )}
        </div>
      </div>
