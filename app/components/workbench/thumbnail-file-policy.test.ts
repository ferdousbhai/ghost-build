...
import { MAX_THUMBNAIL_FILE_BYTES, thumbnailFileValidationError } from './thumbnail-file-policy';

describe('thumbnailFileValidationError', () => {
  it('accepts a non-empty image within the upload limit', () => {
    expect(thumbnailFileValidationError({ type: 'image/png', size: MAX_THUMBNAIL_FILE_BYTES })).toBeNull();
  });
  ...
  it('rejects non-images, empty images, and images above the upload limit', () => {
    ...
    expect(thumbnailFileValidationError({ type: 'image/png', size: MAX_THUMBNAIL_FILE_BYTES + 1 })).toBe(
