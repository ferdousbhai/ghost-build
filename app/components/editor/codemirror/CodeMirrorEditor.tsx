...
import type { EditorUpdate, OnChangeCallback, OnSaveCallback, OnScrollCallback, OnWheelCallback } from './editor-types';
...
export const CodeMirrorEditor = memo(function CodeMirrorEditor({
  ...
  useEffect(() => {
    ...
    const onUpdate = debounce((update: EditorUpdate) => onChangeRef.current?.(update), CHANGE_DEBOUNCE_MS);
  ...
  useEffect(() => {
    ...
    const callbacks = { onScroll: onScrollRef, onWheel: onWheelRef, onSave: onSaveRef };
    if (!currentDocument) {
