...
/**
 * The image every user's workspace container runs: Cloudflare's stock Sandbox image, pulled
 * directly from Docker Hub. The runtime bootstrap installs pnpm and computerd on the first cold
 * container and warms the pnpm store from the project lockfile as it builds. Ghostbuild used to
 * bake those into a per-account image mirrored into `registry.cloudflare.com`, but that ~400 MB
 * per-account copy was the whole "preparing your workspace" wall and its own source of exec and
 * Node-version bugs, for a cold-start speedup that was not worth the machinery. Update the pin
 * here; there is nothing else to republish.
 */
...
export async function provisionUserWorkspaceRuntime(args: {
  ...
  const claim = await claimUserWorkspaceRuntimeProvisioning({
    ...
    // Lets the claim treat an image change as a reason to re-provision, so the staleness check and
    // the claim agree on which image a ready runtime should be on.
    expectedImageDigest: USER_WORKSPACE_SANDBOX_BASE_IMAGE,
  });
  if (!claim.claimed) {
