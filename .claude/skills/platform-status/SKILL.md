...
`wrangler d1 execute ghostbuild --remote --json` under the operator's existing Cloudflare authentication. Every
statement is a `SELECT`; the tool never writes, deletes, or upgrades anything.
