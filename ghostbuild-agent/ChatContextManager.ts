...
const DEFAULT_MAX_COLLAPSED_MESSAGES_SIZE = 160_000;
const DEFAULT_MIN_COLLAPSED_MESSAGES_SIZE = 100_000;
const DEFAULT_MAX_RELEVANT_FILES_SIZE = 80_000;

type UIMessagePart = GhostbuildPart;
...
export class ChatContextManager {
  assistantMessageCache: WeakMap<GhostbuildMessage, ParsedAssistantMessage> = new WeakMap();
  messageSizeCache: WeakMap<GhostbuildMessage, number> = new WeakMap();
   ...
   *
   * 1. The stack guidelines, which are filled in by the server and
   *    may be provider-cacheable depending on the active model.
  ...
  prepareContext(
    ...
    maxCollapsedMessagesSize = DEFAULT_MAX_COLLAPSED_MESSAGES_SIZE,
    minCollapsedMessagesSize = DEFAULT_MIN_COLLAPSED_MESSAGES_SIZE,
  ): { messages: GhostbuildMessage[]; collapsedMessages: boolean; promptCharacterCounts?: PromptCharacterCounts } {
   ...
   */
  calculatePromptCharacterCounts(messages: GhostbuildMessage[], systemPrompts?: string[]): PromptCharacterCounts {
  ...
  private messageSize(message: GhostbuildMessage): number {
    const cached = this.messageSizeCache.get(message);
    if (cached !== undefined) {
      return cached;
    }

    let size = messageText(message).length;
  ...
  relevantFiles(
    messages: GhostbuildMessage[],
    id: string,
    maxRelevantFilesSize = DEFAULT_MAX_RELEVANT_FILES_SIZE,
  ): GhostbuildMessage {
    ...
    for (const message of messages) {
      const createdAt = createdAtMillis(message);
  ...
  private collapseMessages(messages: GhostbuildMessage[]): GhostbuildMessage[] {
    const startIndex = Math.max(this.messageIndex, 0);
    return messages.slice(startIndex).map((message, index) => {
      const messageIndex = index + startIndex;
      if (messageIndex === this.messageIndex) {
        const filteredParts = message.parts.filter((p, j) => {
          return !isToolResult(p) || j > this.partIndex;
        });
        const remainingMessage = {
          ...message,
          content: StreamingMessageParser.stripArtifacts(messageText(message)),
          parts: filteredParts,
        };
      ...
      if (message.role === 'user') {
        for (const part of message.parts) {
          if (part.type === 'text' && part.text.includes('title="Relevant Files"')) {
            ...
            // We used to strip out file content when serializing persisted messages.
            const hasContent =
  ...
  private messagePartCutoff(messages: GhostbuildMessage[], maxCollapsedMessagesSize: number): [number, number] {
    ...
    for (let messageIndex = messages.length - 1; messageIndex >= 0; messageIndex--) {
      ...
      for (let partIndex = message.parts.length - 1; partIndex >= 0; partIndex--) {
        ...
        if (invocation && invocation.state !== 'result') {
          continue;
        }
  ...
  private parsedAssistantMessage(message: GhostbuildMessage): ParsedAssistantMessage | null {
    ...
    for (const file of extractFileArtifacts(makePartId(message.id, 0), messageText(message))) {
    ...
    for (let j = 0; j < message.parts.length; j++) {
      ...
      const invocation = getToolInvocation(part);
      if (invocation?.toolName === 'view' && invocation.state !== 'partial-call') {
  ...
  private partSize(part: UIMessagePart) {
    ...
    switch (part.type) {
      ...
      default: {
        const invocation = getToolInvocation(part);
        if (invocation) {
          result += JSON.stringify(invocation.args).length;
          if (invocation.state === 'result') {
            result += JSON.stringify(invocation.result).length;
          }
          break;
        }
...
function makeUserMessage(content: string[], id: string): GhostbuildMessage {
